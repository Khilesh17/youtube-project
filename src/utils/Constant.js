export const SIDE_BAR_LOGO = "data:image/png;base64,";

export const YOUTUBE_LOGO = "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/YouTube_Logo_2017.svg/2560px-YouTube_Logo_2017.svg.png";

export const PROFILE_LOGO = "https://www.iconpacks.net/icons/2/free-user-icon-3296-thumb.png";

const GOOGLE_API_KEY = "AIzaSyAI8ocnXiMGuEq7iaYM2f7Z7x9TIx-4GBo";

export const YOUTUBE_VIDEOS_API =
    "https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=50&regionCode=IN&key=" +
    GOOGLE_API_KEY;

export const YOUTUBE_SEARCH_API =
    "http://suggestqueries.google.com/complete/search?client=firefox&ds=yt&q=";

export const LIVE_CHAT_COUNT = 25;
// Live Chat >>>> Infinite Scroll >>>>>> Pagination